...
public class LatticeRenderer extends JPanel {
	...
	// Gr��e des Panels zur Darstellung
	...
	public void renderOneDimension() {
		...
		for (int x = 0; x < Lattice.WIDTH - 1; x++) {
			for (int y = 0; y < Lattice.HEIGHT - 1; y++) {
				...
				// Kreis f�r den Knoten = Neuron bef�llen
				g2.setColor(Color.RED);
				g2.fillOval(xpos1 - 2, ypos1 - 2, 4, 4);
				g2.fillOval(xpos4 - 2, ypos4 - 2, 4, 4);

				g2.setColor(Color.black);
				// waagerechte Verbindung von x/y zum n�chsten Knoten x+1/y zeichnen:
				g2.drawLine(xpos1, ypos1, xpos2, ypos2);

				// Verbindung vom letzten Knoten einer Zeile zum ersten der n�chsten Zeile
	...
	// Erzeugt gem�� den Gewichtsvektoren der Knoten mit Farbe bef�llte Rechtecke
	public void renderRGB() {
		// System.out.println (getWidth() + " H�he: " +getHeight());
		// Gr��e pro Zelle: 10 Pixel (400 / 40 = 10)
		int cellWidth = PANELWIDTH / Lattice.WIDTH;
		int cellHeight = PANELHEIGHT / Lattice.HEIGHT;
		// System.out.println (cellWidth + " ZellenH�he: " +cellHeight);
	...
	/*
	 * Yeah, it's ugly, too. Erzeugt gem�� den Gewichtsvektoren der Knoten die
	...
	// IRIS-Daten werden farbig markiert: Gelb f�r die 1. Sorte, Rot f�r die 2.
	// Sorte und Blau f�r die 3. Sorte
	public void renderIRIS() {
		...
		for (int x = 0; x < Lattice.WIDTH; x++) {
			for (int y = 0; y < Lattice.HEIGHT; y++) {
				...
				g2.drawRect((int) (x * cellWidth), (int) (y * cellHeight), (int) cellWidth, (int) cellHeight);
				g2.setColor(Color.WHITE);
				// sauberes Rechteck mit wei�en Zellen initialisieren
				g2.fillRect((int) (x * cellWidth) + 1, (int) (y * cellHeight) + 1, (int) cellWidth - 1,
						(int) cellHeight - 1);
			}
		}
		// Jetzt die Farben auf die Karte: Suche BMU f�r die Daten und f�rbe ein
		try {
			// String workingDir = System.getProperty("user.dir");
			URL url = this.getClass().getResource("IrisDataSet.txt");
	...
	// Falls man mit der Maus �ber einen Knoten geht, wird der Inhalt des
