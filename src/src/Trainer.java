...
public class Trainer implements Runnable {
	...
	private final int NUM_ITERATIONS = 16 * Lattice.HEIGHT * Lattice.WIDTH;
	...
	public MyVector getTrainingData() {
		...
		// F�r Rechteck als Input erzeuge zweidimensionale Zufallszahl
		...
		if (myDATA == DATA.CIRCLE) {
			// hier die Erzeugung von Trainingsdaten erg�nzen
			...
			do {
				x = Math.random();
				y = Math.random();
				if(Math.sqrt(Math.pow(x-0.5, 2) + Math.pow(y-0.5, 2)) >= 0.5) recalc = true;
				else recalc = false;
			} while(recalc);
		...
		if (myDATA == DATA.RING) {
			...
			do {
				x = Math.random();
				y = Math.random();
				double d = Math.sqrt(Math.pow(x-0.5, 2) + Math.pow(y-0.5, 2));
				if(d >= 0.5 || d < 0.3) recalc = true;
				else recalc = false;
			} while(recalc);
		...
		if (myDATA == DATA.SHAPE) {
			...
			do {
				x = Math.random();
				if(x < 0.8 && x > 0.65 || x > 0.2 && x < 0.35) recalc = true;
				else recalc = false;
			} while(recalc);

			do {
				y = Math.random();
				if(y < 0.8 && y > 0.65 || y > 0.2 && y < 0.35) recalc = true;
				else recalc = false;
			} while(recalc);
		...
		if (myDATA == DATA.RGB_SEED_9_COLORS || myDATA == DATA.RGB_SEED_RED || myDATA == DATA.IRIS) {
			...
			// w�hle eine der festen Vektoren aus den Inputdaten
	...
	public void initTrainingData() {
		...
		if (myDATA == DATA.RGB_SEED_RED) {
			// Mehrere Schattierungen von "Rot" als Seed w�hlen
			tempVec = new MyVector();
			tempVec.add(1.0);
			tempVec.add(0.0);
			tempVec.add(0.0);
	...
	public void run() {
		...
		while (iteration < NUM_ITERATIONS && running) {
			...
			// F�r jeden Eingabevektor suche die "best matching unit"
			...
			for (int x = 0; x < Lattice.WIDTH && myDATA != DATA.TWO_TO_ONE_DIM; x++) {
				for (int y = 0; y < Lattice.HEIGHT; y++) {
					...
					if (dist <= nbhRadius) {
						// Gewichte in der N�he des Gewinnerneurons st�rker anpassen
